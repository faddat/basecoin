...
func (suite *KeeperTestSuite) TestMigrator_Migrate3to4() {
	...
	for _, def := range []bool{true, false} {
		...
		suite.T().Run(fmt.Sprintf("default %t send enabled info moved to store", def), func(t *testing.T) {
			...
			require.Len(newParams.SendEnabled, 0) //nolint:staticcheck // SA1019: banktypes.Params.SendEnabled is deprecated: Use SendEnabledEntries instead.
			require.Equal(def, newParams.DefaultSendEnabled)

			for _, se := range params.SendEnabled { //nolint:staticcheck // SA1019: banktypes.Params.SendEnabled is deprecated: Use SendEnabledEntries instead.
				actual := bankKeeper.IsSendEnabledDenom(ctx, se.Denom)
				require.Equal(se.Enabled, actual, se.Denom)
			}
		})
...
func (suite *KeeperTestSuite) TestSetParams() {
	...
	params.SendEnabled = []*banktypes.SendEnabled{ //nolint:staticcheck // SA1019: banktypes.Params.SendEnabled is deprecated: Use SendEnabledEntries instead.
	...
	suite.Run("stored params are as expected", func() {
		...
		require.Len(actual.SendEnabled, 0, "SendEnabled") //nolint:staticcheck // SA1019: banktypes.Params.SendEnabled is deprecated: Use SendEnabledEntries instead.
	})
